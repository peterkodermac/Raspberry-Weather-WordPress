 ...
 * Author: Peter Kodermac - contributions by A. Catorcini
...
//adding the option to have barometric pressure too 
     ...
     $resultSet = mysql_query("SELECT " . $display . " FROM temperatures WHERE dateMeasured=DATE(NOW())" );
     ...
     if (mysql_num_rows($resultSet) == 0) //nothing in table
           ...
           { //what needs to be displayed - temps, pressure or humidity or all
     ...
     while ($row = mysql_fetch_assoc($resultSet)) {
           ...
           else if (strpos($display, "pressure") != 0) //for displaying pressure only

                $content .= "['" . gmdate("H:i", ($hourMeasured * 60)) . "'," . $row['pressure'] . "],";

           else

                $content .= "['" . gmdate("H:i", ($hourMeasured * 60)) . "'," . $temperature . "," . $row['humidity']. $row['pressure'] . "],";
